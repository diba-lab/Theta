classdef SDFigures2 <Singleton
    ...
    methods
        ...
        function plotFooof(obj, plotwh)
            ...
            selected_ses = [1:2, 4:12, 14:15, 18:23];
            tses = sf.getSessionsTable(selected_ses);
            % ticdUpdate(tses.Filepath,tses.ZT)
            sde = experiment.SDExperiment.instance.get;
            ...
            if ~isfile(cacheFile)
                ...
                for icond = 1:numel(conditionsCat)
                    ...
                    for isession = 1:numel(tses_cond)
                        ...
                        sdd = buzcode.sleepDetection.StateDetectionData(file);
                        if isempty(sdd.TimeIntervalCombinedOriginal.getZeitgeberTime)
                            sdd.TimeIntervalCombinedOriginal=sdd.TimeIntervalCombinedOriginal.setZeitgeberTime(ses.getZeitgeberTime);
                            sdd.TimeIntervalCombinedOriginal.saveTable;
                        end
                        EMG = sdd.getEMG.getReSampled(1250);
                        ss = sdd.getStateSeries;
                        thId = sdd.getThetaChannelID;
                        ctd = neuro.basic.ChannelTimeDataHard(file);
                        th = ctd.getChannel(thId).getDownSampled(250);
                        blocks = ses.Blocks;
                        blocksStr = categorical(blocks.getBlockNames);

                        for iblock = 1:numel(blocksStr)
                            try
                                boc = neuro.basic.BlockOfChannels();
                                boc.Info.Condition = cond;
                                boc.Info.Session = isession;
                                ...
                                timeWindowadj = obj.adjustTimeWindow(timeWindow, winDuration);
                                allBlock = th.getTimeWindow(timeWindowadj);
                                boc = boc.addChannel(allBlock).addChannel(EMG);
                                ...
                                for istate = 1:numel(statelist)
                                    try
                                        ...
                                        cacheFilePower = fullfile(sde.FileLocations.General.PlotFolder, ...
                                            'Cache', DataHash(params), ...
                                            strcat(sprintf('PlotFooof_afoof_%s_%s_%s_%s_', ...
                                            cond, matlab.lang.makeValidName(ses.toString), block, thestate), '.mat'));

                                        % Create folder if it does not exist
                                        folderPath = fileparts(cacheFilePower);
                                        ...
                                        if isfile(cacheFilePower)
                                            ...
                                            try
                                                [thpks, epiFooof] = obj.processSubBlocks(boc, params, thestate, edges);
                                                save(cacheFilePower, 'thpks', 'epiFooof');
                                            catch ME
                                                warning(ME.identifier, 'Error processing sub-blocks: %s', ME.message);
                                            end
                                        end
                                    catch ME
                                        warning(ME.identifier, 'Error processing state %s: %s', char(thestate), ME.message);
                                    end
                                end
                                stateRatiosInTime = ss_block.getStateRatios(minutes(30), minutes(3));
                                Cond{iblock, isession, icond} = stateRatiosInTime;
                            catch ME
                                warning(ME.identifier, 'Error processing block %s: %s', char(block), ME.message);
                            end
        ...
        function timeWindowadj = adjustTimeWindow(~, timeWindow, winDuration)
            if winDuration > 0
                timeWindowadj = [timeWindow(1), timeWindow(1) + hours(winDuration)];
                if timeWindowadj(2) > timeWindow(2)
                    timeWindowadj(2) = timeWindow(2);
        ...
        function [thpks, epiFooof] = processSubBlocks(obj, boc, params, thestate, edges)
            subblocks = boc.getStartTime + seconds(edges);
            for isublock = 1:(numel(subblocks) - 1)
                subblock = subblocks([isublock, isublock + 1]);
                subblock = obj.adjustSubBlock(subblock, boc);

                if subblock(2) > subblock(1)
                    boc_sub = boc.getWindow(subblock);
                    [thpks, epiFooof] = obj.processEpisode(boc_sub, params, thestate, isublock);
                end
            end
        end

        function subblock = adjustSubBlock(~, subblock, boc)
        ...
        function [thpks, epiFooof] = processEpisode(~, boc_sub, params, thestate, ~)
            ...
            if ~isempty(boc_sub)
                ...
                if ~isempty(episode) && (episode{1}.getLength > minutes(params.Plot.MinDurationInSubBlockMinutes))
                    ...
                    fooof = episode{1}.getPSpectrumWelch.getFooof(params.Fooof(1), params.Fooof(1).f_range);
                    fooof.Info = episode{1}.Info;
                    fooof.Info.episode = episode{1};
                    thpk = thpk.addFooof(fooof);
                    thpk.Bouts = array2table(theEpisodeAbs, 'VariableNames', {'startAbs', 'endAbs'});
        ...
        function thpkc=getThetaPeaks(obj,blocks)
                    ...
                    for iblock=1:numel(blocks)
                        ...
                        for istate=1:numel(states)
                            ...
                            cacheFilePower = fullfile(sdeparams.FileLocations.General.PlotFolder, ...
                                'Cache', DataHash(params), ...
                                strcat(sprintf('PlotFooof_afoof_%s_%s_%s_%s_', ...
                                cond, matlab.lang.makeValidName(ses.toString), block, state), '.mat'));
                            if isfile(cacheFilePower)
            ...
            thpkc=experiment.plot.thetaPeak.ThetaPeaksContainer( ...
                thpks,fooof,params);
        end
        ...
        function plot_FooofInBlocks_CompareConditions(obj,Conds,plotwh)
            ...
            for istate=1:numel(statelist)
                ...
                for iblock=1:size(Conds,2)
                    ...
                    for iplot=1:2
                        try
                            perrbar=errorbar(centers, meanval(:,iplot), ...
                                errval(:,iplot), ...
                        ...
                        thecolor=colors{iplot};
                        plot(centers+centershift(iplot),varSes,'Color', ...
                            thecolor, ...
                ...
                switch plotwh
                    case 1 %CF
                        ...
                        ff.save(strcat('Center Frequency_',char( ...
                            statelist1(istate))));
                    case 2 %power
                        ax.YLim=[0 1.2];
        ...
        function axs=plotEpisode(obj,boc,params,thestate)
            ...
            if ~isempty(episode)
                ...
                tfm=episode1.getWhitened.getTimeFrequencyMap(...
                    ...
                    logspace(log10(thetaFreq(1)),log10(thetaFreq(2)), ...
                    diff(thetaFreq)*5)));
                ...
                if ~isempty(tfm.timePoints)
                    ...
                    %                               thpk.plot('Color','r');
                    thpkcf_fd=thpkcf.getMedianFiltered(1,'omitnan','truncate' ...
                        ).getMeanFiltered(1);
                    ...
                    durations1=cumsum(seconds(theEpisodeAbs(:,2)- ...
                        theEpisodeAbs(:,1)))';
                    durations2=[0 durations1];
                    durationscenter=durations2(1:(numel(durations2)-1))+diff( ...
                        durations2)/2;
                    ...
                    if episode1.getLength>minutes( ...
                            params.Plot.MinDurationInSubBlockMinutes)
                        fooof=episode1.getPSpectrumWelch.getFooof( ...
                            params.Fooof(2), ...
                ...
                if ~isempty(hyp.States)
                    hyp.plot([0 1])
                    zttime=hours(theEpisodeAbs- ...
                        hyp.TimeIntervalCombined.getZeitgeberTime);
                    theEpisodeCenters=mean(zttime,2);
                    for ib=1:size(zttime,1)
                        t1=text(theEpisodeCenters(ib),mean(axn.YLim), ...
                            num2str(ib));
            ...
            if  ~isempty(episode)&&~isempty(tfm.timePoints)
                ...
                annotation('textbox',[0 .65 .3 .3],'String',str, ...
                    'FitBoxToText','on');
