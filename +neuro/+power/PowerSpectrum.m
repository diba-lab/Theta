classdef PowerSpectrum
    ...
    methods
        ...
        function [freq, pwr]=getPeak(powerSpectrum,f_range)
            ...
            rmpath('external\chronux\spectral_analysis\continuous');
            [pks,locs] = findpeaks(power,'SortStr','descend');
            if exist('external\chronux\spectral_analysis\continuous', 'dir')
                addpath('external\chronux\spectral_analysis\continuous');  % Add it back later if needed
            end
            if ~isempty(pks)
